/*
 * $Id: tdcOsSock.c 11619 2010-07-22 10:04:00Z bloehr $
 *
...
// ----------------------------------------------------------------------------
   ...
   #error "This is the INTEGRITY Version of tdcOsSock, "
...
void tdcInitCli_SERV_ADDR (const char*             pModname,
                           ...
                           UINT16                  portNo)
{
   if((pServAddr == NULL) || (pLen == NULL))
   {
      DEBUG_WARN (pModname, "tdcInitCli_SERV_ADDR: Wrong parameter (NULL)");
      return;
   }
                           ...
                           UINT16                  portNo)
{
   if((pServAddr == NULL) || (pLen == NULL))
   {
      DEBUG_WARN (pModname, "tdcInitSrv_SERV_ADDR: Wrong parameter (NULL)");
      return;
   }
                    ...
                    int                      sockAddrSize)
{
   if(pServerAddr == NULL)
   {
      DEBUG_WARN (MOD_MAIN, "Connect: Wrong parameter (NULL)");
      return FALSE;
   }
                               ...
                               struct timeval*     pTimeout)
{
  ...
  if((readFdSet == NULL) || (writeFdSet == NULL) || 
     (exceptFdSet == NULL) || (pTimeout == NULL))
  {
     DEBUG_WARN (MOD_MAIN, "Select: Wrong parameter (NULL)");
     return 0;
  }
                        ...
                        UINT32                     timeout)
{
   ...
   if(pServerAddr == NULL)
   {
      DEBUG_WARN (MOD_MAIN, "ConnectTimo: Wrong parameter (NULL)");
      return FALSE;
   }
                 ...
                 int                      addrLen)
{
   if(pMyAddr == NULL)
   {
      DEBUG_WARN (MOD_MAIN, "Bind: Wrong parameter (NULL)");
   }
                     ...
                     T_TDC_SOCKLEN_T*     pSaLen)
{
   ...
   {
      DEBUG_WARN (pModname, "Accept: Wrong parameter (NULL)");
      if (strcmp(pModname, MOD_IPC) != 0)
      {
        return connFd;   
      }
   }
...
{
   if((pSendBuf == NULL) || (pBytesSent == NULL))
   {
      DEBUG_WARN (pModname, "tdcTcpSendN: Wrong parameter (NULL)");
      return FALSE;
   }
...
T_TDC_BOOL tdcTcpRecvN (const char*       pModname,
                        ...
                        UINT32            bytesExpected)
{
   if((pRecvBuf == NULL) || (pBytesReceived == NULL))
   {
      DEBUG_WARN (pModname, "tdcTcpRecvN: Wrong parameter (NULL)");
      return FALSE;
   }
                        ...
                        T_TDC_SOCK_ADDR_IN*  pSockAddr)
{
   ...
   if((pServAddr == NULL) || (pSockAddr == NULL))
   {
      DEBUG_WARN (MOD_MAIN, "getInetAddr: Wrong parameter (NULL)");
      return FALSE;
   }
                              ...
                              UINT16            portNo)
{
   ...
   if(pServerSockFd == NULL)
   {
      DEBUG_WARN (pModname, "tdcServerUdpSock4: Wrong parameter (NULL)");
      return FALSE;
   }
                               ...
                               UINT16              portNo)
{
   ...
   if(pServerSockFd == NULL)
   {
      DEBUG_WARN (pModname, "tdcServerUdpSock46: Wrong parameter (NULL)");
      return FALSE;
   }
                               ...
                               T_TDC_SOCKLEN_T*       pServerSockLen)
{
   ...
   if((pClientSockFd == NULL) || (pRemAddr == NULL) || 
      (ppServerAddr == NULL) || (pServerSockLen == NULL))
   {
      DEBUG_WARN (pModname, "tdcClientUdpSock46: Wrong parameter (NULL)");
      return FALSE;
   }
   ...
   {                                                                                                  /*@ -type */
      ...
      if (clientSockFd >= 0)            /* success */
      {
         ...
         *pServerSockLen = pRes->ai_addrlen;    /*@ =type */            /*@ -type */
         if(*ppServerAddr == NULL)
         {
            DEBUG_WARN (pModname, "tdcClientUdpSock46: Wrong parameter *ppServerAddr (NULL)");
            return     (FALSE);
         }
...
void Inet_pton (int family, const char*   pServerName, void*   pServerAddr)
{
   ...
   if((pServerName == NULL) || (pServerAddr == NULL))
   {
      DEBUG_WARN (MOD_MAIN, "Inet_pton: Wrong parameter (NULL)");
      return;
   }
                              ...
                              T_TDC_SOCKLEN_T*        pServSockLen)
{
   ...
   if((pServSockFd == NULL) || (pRemAddr == NULL) || 
      (ppServAddr == NULL) || (pServSockLen == NULL))
   {
      DEBUG_WARN (pModname, "tdcClientUdpSock4: Wrong parameter (NULL)");
      return FALSE;
   }
   ...
   {                                                                 /*@ -type */
      ...
      if(*ppServAddr == NULL)
      {
         DEBUG_WARN (pModname, "tdcClientUdpSock4: Wrong parameter *ppServAddr (NULL)");
         return FALSE;
      }
...
{
   if((pSendBuf == NULL) || (pServerAddr == NULL))
   {
      DEBUG_WARN (pModname, "tdcUdpSendN: Wrong parameter (NULL)");
      return FALSE;
   }
                        ...
                        UINT16            bytesExpected)
{
   if(pRecvBuf == NULL)
   {
      DEBUG_WARN (pModname, "tdcUdpRecvN: Wrong parameter (NULL)");
      return FALSE;
   }
