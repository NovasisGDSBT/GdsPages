//
// $Id: tdcOsSema.c 11726 2010-11-25 09:49:21Z gweiss $
//
...
#if !defined (WIN32)
   #error "This is the Windows Version of tdcOsSema, i.e. WIN32 has to be specified"
#endif
                                 ...
                                 T_TDC_MUTEX_ID    mutexId)
{
   ...
   if (mutexStatus != TDC_MUTEX_OK)
   {
      ...
      (void) tdcSNPrintf (text, (unsigned int) sizeof (text), "Unable to lock mutex (%s)", pMutexName);
      text[sizeof (text) - 1] = '\0';
      DEBUG_ERROR (pModName, text);
   }
                                 ...
                                 T_TDC_SEMA_ID  semaId)
{
   ...
   if (semaStatus != TDC_SEMA_OK)
   {
      ...
      text[sizeof (text) - 1] = '\0';
           // CR-685: don't use ERROR since this leads to abnomal termination, and this warning will
           // appear during termination
